...
const selectedEmployees = defineModel<string[]>();
...
// // run: 手动执行请求的方法
// // loading: 请求加载状态
// const { run: fetchEmployeeList, loading } = useRequest(
...
// 存储所有已选择的员工ID
const selectedEmployeesSet = ref(new Set<string>(selectedEmployees?.value || []));

// 监听selectedEmployees的变化
watch(() => selectedEmployees?.value, (newValue) => {
  if (newValue) {
    selectedEmployeesSet.value = new Set<string>(newValue);
    // 同步表格选中状态
    if (multipleTableRef.value) {
      employeeList.value.forEach(employee => {
        multipleTableRef.value?.toggleRowSelection(employee, selectedEmployeesSet.value.has(employee.userId));
      });
    }
  }
}, { immediate: true });
...
// 处理表格选择变化事件
const handleSelectionChange = (selection: EmployeeInfo[]) => {
  ...
  currentPageEmployeeIds.forEach(id => {
    if (!selection.some(employee => employee.userId === id)) {
      selectedEmployeesSet.value.delete(id);
    }
  });

  // 添加新选中的员工到selectedEmployeesSet
  selection.forEach(employee => {
    selectedEmployeesSet.value.add(employee.userId);
  });

  // 更新selectedEmployees的值
  selectedEmployees.value = Array.from(selectedEmployeesSet.value);
};
  ...
  userService.getEmployeeList,
  {
    onSuccess: data => {
      ...
      nextTick(() => {
        ...
        if (multipleTableRef.value) {
          employeeList.value.forEach(employee => {
            if (selectedEmployeesSet.value.has(employee.userId)) {
              multipleTableRef.value?.toggleRowSelection(employee, true);
            }
          });
        }
